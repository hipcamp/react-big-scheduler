    ...
    value: function render() {
      ...
      var tableRows = displayRenderData.map(function (item) {
        var rowCells = headers.map(function (header, index) {
          ...
          var isToday = new Date(header.time).toLocaleDateString() === new Date().toLocaleDateString();
          if (!!header.nonWorkingTime) {
            style = _extends({}, style, { backgroundColor: config.nonWorkingTimeBodyBgColor });
          }
          if (item.groupOnly) {
          ...
          if (!!behaviors.getNonAgendaViewBodyCellBgColorFunc) {
            ...
            if (!!cellBgColor) {
              style = _extends({}, style, { backgroundColor: cellBgColor });
            }
          }
          return _react2.default.createElement('td', { key: key, style: style, className: className }, _react2.default.createElement('div', null));
        });

        return _react2.default.createElement('tr', { key: item.slotId, style: { height: item.rowHeight } }, rowCells);
      });

      return _react2.default.createElement('tbody', null, tableRows);
    }
  }]);
